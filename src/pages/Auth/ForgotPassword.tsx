...
const LoaderOverlay: React.FC<{ onCancel: () => void }> = ({ onCancel }) => (
  <div className="absolute inset-0 flex items-center justify-center bg-[#121212] bg-opacity-95 z-50">
    <div className="flex flex-col items-center justify-center space-y-4">
      ...
      <div className="text-white text-xl opacity-80 mt-8 font-roboto">
        Sending reset link...
      </div>
      <button
        onClick={onCancel}
        className="text-gray-400 text-sm font-roboto underline hover:text-[#3CE7B2] transition-colors duration-200"
      >
        Cancel
      </button>
    </div>
    ...
    <div className="min-h-screen w-full bg-[#121212] flex flex-col items-center justify-center relative font-roboto">
      ...
      {success ? (
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          />
          <h1 className="text-4xl font-bold text-white font-roboto mix-blend-difference">
            Check your email
          </h1>
          <p className="text-md text-center text-gray-400 font-roboto">{email}</p>
          <p className="text-sm text-gray-400 mt-2 font-roboto">{success}</p>
        </div>
      ) : (
        <div className="w-full max-w-md bg-[#1D1D1D] border border-[#2C2C2C] rounded-xl p-6 shadow-lg hover:border-gray-700 transition-all duration-300">
          <h1 className="text-4xl font-bold text-white font-roboto mix-blend-difference mb-6 text-center">
            Forgot Password
          </h1>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label
                htmlFor="email"
                className="text-sm text-white font-roboto block"
              >
                Email
              </label>
              <input
              ...
              />
              {error && (
                <p className="text-[#3CE7B2] text-sm font-roboto text-center mt-2">
                  {error}
                </p>
              )}
            </div>
            <button
              type="submit"
              className="w-full py-3 bg-[#3CE7B2] text-[#121212] rounded-md hover:bg-[#27A98B] transition-colors duration-200 font-roboto disabled:bg-[#2C2C2C] disabled:text-gray-400"
              disabled={loading}
            >
              Reset Password
            </button>
