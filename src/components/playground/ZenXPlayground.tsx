...
const ZenXPlayground: React.FC<ZenXPlaygroundProps> = ({ propsProblemID, hideBackButton }) => {
  ...
  const { data: problemData, isLoading: isProblemLoading } = useProblemById(propsProblemID || '');

  // Use effect to track propsProblemID changes
  useEffect(() => {
    if (propsProblemID && propsProblemID !== problemId) {
      setProblemId(propsProblemID);
    }
  }, [propsProblemID]);
  ...
  // Update problem when problemData changes from useProblemById
  useEffect(() => {
    if (problemData && !isProblemLoading) {
      setProblem(problemData);
    }
  }, [problemData, isProblemLoading]);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-full bg-zinc-950 text-zinc-300">
  ...
  return (
    <div className={`bg-zinc-950 flex flex-col ${hideBackButton ? 'p-0' : 'min-h-screen'}`}>
      ...
      <div className="flex-1 overflow-hidden">
        <ResizablePanelGroup
          ...
          className={hideBackButton ? "" : "min-h-[calc(100vh-3rem)]"}
        >
          {!isMobile && (
            <>
              <ResizablePanel
              ...
              >
                <ProblemDescription problem={problem} hideBackButton={hideBackButton} />
              </ResizablePanel>
...
// Updated ProblemDescription component to include hideBackButton prop
          ...
          <div>
            ...
            {executionResult && (
              <div className="mt-4 p-3 bg-zinc-900/70 border border-zinc-800/80 rounded-md">
                ...
                {executionResult.failedTestCase?.testCase
